...
interface DashboardProps {
  goApp: (screen: string) => void;
  setQueueTab: (tab: string) => void;
  selectedSites?: string[];
  setHighlightArtIds: (ids: string[]) => void;
}
...
export function Dashboard({
  ...
  const totalArts    = siteArts.length;
  const pendingArts  = siteArts.filter(a => a.status !== 'green').length;
  const approvedArts = siteArts.filter(a => a.status === 'green').length;
  const approvalRate = totalArts === 0
    ? '—'
    : Math.round(approvedArts / totalArts * 100) + '%';

  const statCards = [
    { label: 'TOTAL ARTICLES', val: String(totalArts),    accent: C.info, color: C.fg },
    { label: 'PENDING REVIEW', val: String(pendingArts),  accent: C.pr,   color: C.fg },
    { label: 'APPROVED',       val: String(approvedArts), accent: C.gr,   color: C.gr },
    { label: 'APPROVAL RATE',  val: approvalRate,         accent: C.pr,   color: C.fg },
  ];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ backgroundColor: C.page }}>
      ...
      <div
        ...
        style={{ height: 72, backgroundColor: C.page, borderBottom: `1px solid ${C.border}` }}
      >
        <div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span style={{ ...T.small }}>
                  {getWeekLabel(weekOffset)} · Total: {chartTotal}
                </span>
                {/* Prev / Next arrows */}
                <div
                  className="flex items-center"
                  style={{ borderRadius: 6, border: `1px solid ${C.border}`, overflow: 'hidden' }}
                >
                  <button
                    onClick={() => setWeekOffset(o => Math.max(-11, o - 1))}
                    ...
                    onMouseEnter={e => {
                      if (weekOffset > -11) {
                        (e.currentTarget as HTMLButtonElement).style.backgroundColor = C.surfHov;
                    ...
                    onMouseLeave={e => {
                      if (weekOffset > -11) {
                        ...
                        (e.currentTarget as HTMLButtonElement).style.color = C.mutedFg;
                      }
                    }}
                  >
                    <ChevronLeft size={12} />
            ...
            <div className="flex-1 px-4 pb-4 pt-3 min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    tickCount={8}
                    domain={[0, (dataMax: number) => Math.ceil(dataMax / 7) * 7]}
                    tick={{ fontSize: 9, fill: C.mutedFg }}
                    axisLine={false}
                    tickLine={false}
                    width={20}
                  />
                  <RTooltip
                    ...
                    contentStyle={{
                      ...
                      border: `1px solid ${C.border}`,
                      boxShadow: '0 4px 8px rgba(26,26,26,0.08)',
                      backgroundColor: C.card,
                      padding: '6px 10px',
                    }}
                  ...
                  <Bar dataKey="count" radius={[5, 5, 0, 0]} animationDuration={600} animationEasing="ease-out">
                    {chartData.map((d, i) => (
                      <Cell
                        key={`cell-${i}`}
                        fill={weekOffset === 0 && i === 6 ? C.pr : C.prBdr}
                      />
                    ))}
                  </Bar>
                </BarChart>
          ...
          <div className="flex flex-col gap-3" style={{ height: '75%' }}>
            ...
            >
              <div className="flex-shrink-0 px-4 py-2.5" style={{ borderBottom: `1px solid ${C.border}` }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: C.mutedFg, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  Daily Reviewed
                </span>
              </div>
              <div className="flex-1 flex flex-col">
                ...
                >
                  ...
                  <p style={{ fontSize: 28, fontWeight: 700, color: C.fg, letterSpacing: '-0.02em', lineHeight: 1 }}>
                    10.3
                  </p>
                </div>
                <div className="flex-1 flex flex-col justify-center px-5 py-4">
                  <p style={{ ...T.kpiLabel, marginBottom: 6 }}>Today Approved</p>
                  <p style={{ fontSize: 28, fontWeight: 700, color: C.pr, letterSpacing: '-0.02em', lineHeight: 1 }}>
                    14
                  </p>
                </div>
              </div>
            ...
            >
              <div>
                ...
                <p style={{ fontSize: 13, fontWeight: 400, color: C.ink2, lineHeight: 1.6 }}>
                  {pendingArts} articles need your attention, review them now.
                </p>
              </div>
              <ChevronRight size={14} style={{ color: C.am, flexShrink: 0, marginLeft: 12 }} />
            </div>

            {/* Incremental Article banner */}
            <div
              className="flex-shrink-0 flex items-center justify-between px-4 py-4 cursor-pointer"
              onClick={() => {
                const ids = ARTS.filter(a => a.at?.startsWith('16 May')).slice(0, 3).map(a => a.id);
